...
function renderProgress() {
  ...
  fill.style.background = allDone ? '#22C55E' : 'var(--progress-fill)';

  document.getElementById('progress-text').textContent =
    `${done} / ${total} 완료 (${percent}%)${allDone ? ' 🎉' : ''}`;
...
function setFilter(category) {
  ...
  if (titleEl) titleEl.textContent = SECTION_TITLES[category] ?? '할 일';
